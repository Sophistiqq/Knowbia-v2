<script lang="ts">
  ...
  async function fetchAssessments() {
    ...
    questions_and_answers = data.questions_and_answers.map((qa: any) => ({
      ...
      answers: JSON.parse(qa.answers) // Parse the answers here
    }))
                  ...
                  {#each qa.questions as q, index}
                    {#if q.id === selectedQuestion.id}
                      <div class="responses-wrapper">
                        ...
                        <p>
                          Answer:
                          {#if qa.answers[index] == null}
                            No answer provided
                          {:else if selectedQuestion.type === 'multiple_choice'}
                            {selectedQuestion.options[qa.answers[index]]}
                          {:else if selectedQuestion.type === 'true_false'}
                            {qa.answers[index] === true ? 'True' : 'False'}
                          {:else}
                            {qa.answers[index]}
                          {/if}
                        </p>
                        <p>
                          Points:
                          {selectedQuestion.correctAnswers?.includes(qa.answers[index])
            ...
            {#each students as student}
              {#if questions_and_answers.some((qa) => qa.student_number === student.student_number && qa.assessment_id === selectedAssessment.id)}
                <button class="students" on:click={() => (selectedStudent = student)}>
                  <p>
                    {student.first_name}
                    {student.last_name} ({student.student_number})
                  </p>
                </button>
              {/if}
            {/each}
